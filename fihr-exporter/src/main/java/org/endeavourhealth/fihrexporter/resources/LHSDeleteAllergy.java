...
public class LHSDeleteAllergy {
    public String Run(Repository repository, String baseURL)  throws SQLException {
        ...
        List<List<String>> ids = repository.getDeleteRowsAllergy();
        ...
        for(List<String> rec : ids)
        {
            if (isTrue(repository.Stop())) {
                System.out.println("STOPPING DELETE ALLERGY");
                return "1";
            }
            ...
            if(!rec.isEmpty()) {
                ...
                responseCode=send.Delete(repository, id, "AllergyIntolerance", "0", 4);
