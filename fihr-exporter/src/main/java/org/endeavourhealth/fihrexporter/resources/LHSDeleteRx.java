...
public class LHSDeleteRx {
    public String Run(Repository repository, String baseURL)  throws SQLException {
        ...
        List<List<String>> ids = repository.getDeleteRowsRx();
        ...
        for(List<String> rec : ids)
        {
            if (isTrue(repository.Stop())) {
                System.out.println("STOPPING DELETE RX");
                return "1";
            }
            ...
            if(!rec.isEmpty()) {
                ...
                responseCode=send.Delete(repository, id, "MedicationStatement", "0", 10);
